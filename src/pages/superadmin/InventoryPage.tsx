...
const SuperAdminInventoryPage: React.FC = () => {
  ...
  const getVendorHistory = (productId: string) => {
    const updates = stockUpdates
      ...
      .map(update => getVendorName(update.vendorId))
      .filter(name => name);
    
    return [...new Set(updates)].join(', ') || '-';
  };
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {stockUpdates.map(update => (
                    <tr key={update.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {update.quantity > 0 ? getVendorName(update.vendorId) : '-'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    ...
                    {filteredProducts.map(product => (
                      <tr key={product.id}>
                        ...
                        <td className="px-6 py-4">
                          <div className="text-sm text-gray-500">{getVendorHistory(product.id)}</div>
                        </td>
