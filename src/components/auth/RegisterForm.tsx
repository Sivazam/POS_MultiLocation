...
const RegisterForm: React.FC<RegisterFormProps> = ({
  ...
  const [role, setRole] = useState<UserRole>(isSuperAdmin ? 'superadmin' : 'salesperson');
  ...
  useEffect(() => {
    setRole(isSuperAdmin ? 'superadmin' : 'salesperson');
  }, [isSuperAdmin]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isSuperAdmin || role === 'superadmin') {
        ...
        // For other roles, use the current franchise ID
        if (!franchise) {
      ...
      <div className="pt-2">
        <Button
          ...
          disabled={
            ...
            showLocationWarning ||
            (role === 'salesperson' && !locationId && !isSuperAdmin)
          }
        >
